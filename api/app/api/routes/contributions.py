...
@router.get("/{id}", response_model=ContributionPublic)
def read_contribution_by_id(
    ...
    """
    contribution = crud.get_contribution_by_id(session=session, id=id)# session.get(Contribution, contribution_id)
    if not contribution:
        raise HTTPException(status_code=404, detail="Contribution not found")
    if not current_user.is_superuser and (contribution.owner_id != current_user.id):
        raise HTTPException(status_code=400, detail="Not enough permissions to access Contribution")
    return contribution


@router.post(
    "/", dependencies=[Depends(get_current_active_superuser)], response_model=ContributionPublic
)
...
def delete_contribution(session: SessionDep, id: uuid.UUID, current_user: CurrentUser) -> Any:
    """
    Delete contribution.
    """
    existing_contribution = crud.get_contribution_by_id(session=session, contribution_id=id) # def get_contribution_by_id(*, session: Session, contribution_id: uuid.UUID) -> User | None:
    if not current_user.is_superuser and (existing_contribution.owner_id != current_user.id):
...
def update_contribution(
    *,
    ...
    db_contribution = session.get(Contribution, id)
    if not current_user.is_superuser and (db_contribution.owner_id != current_user.id):
        raise HTTPException(status_code=403, detail="Not enough permissions to update this Contribution")
    if not db_contribution:
        raise HTTPException(
            status_code=404,
            detail=("ERROR Updating Contribution - The contribution with this id : [%s] does not exist in the system", id),
        )
